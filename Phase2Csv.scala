...
abstract class Phase2Csv[T] extends Logging {
  ...
  /**
    * Specify the prefixes for each of the sets of DIOCM metadata.
    */
  protected val dicomHeaderPrefixList: Seq[String] = Seq()
  ...
  private def getDicomColList: Seq[CsvCol[T]] = {
    ???
  }

  private def makeDicomHeader: String = {
    getDicomColList.map(_.header).mkString(",")
  }

  private def makeDicomCsvRow(dataSet: T): String = {
    getDicomColList.map(_.toText(dataSet)).mkString(",")
  }
  ...
  def makeHeader(): String = {
    ...
    val dicomHeader = makeDicomHeader

    val list = Seq(prefixCsv.headerText, dataHeaderList, machineDescriptionCsv.headerText, dicomHeader).filter(_.nonEmpty)
    list.mkString(",")
  }
  ...
  private def makeCsvRow(dataList: Seq[T], dataIndex: Int, machine: Machine, mtMachList: Seq[MaintenanceRecord]): String = {
    try {
      ...
      val dicomText = makeDicomCsvRow(dataSet)

      val csvRow = {
        val list = Seq(prefixText, dataText, machineDescriptionText, dicomText).filter(_.size > 0)
