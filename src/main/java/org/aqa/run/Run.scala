...
object Run extends Logging {
  ...
  private def startProcessMonitor(activeProcess: ActiveProcess, msTimeout: Long): Unit = {
    def run(): Unit = {
      managed(activeProcess.logger) acquireAndGet { lgr =>
        {
          try {
  ...
  private def machTypeEnv(mt: MachineType): EnvVal = {
    Map(("mt_manufacturer" -> mt.manufacturer), ("mt_model" -> mt.model), ("mt_version" -> mt.version), ("mt_notes" -> mt.notes))
  }
  ...
  private def machBeamEnergyToEnv(mbeList: Seq[MachineBeamEnergy]): EnvVal = {
    def mapMBE(mbe: MachineBeamEnergy, index: Int): EnvVal = {
      ...
      Seq(mm("photonEnergy_MeV", mbe.photonEnergy_MeV), mm("maxDoseRate_MUperMin", mbe.maxDoseRate_MUperMin), mm("fffEnergy_MeV", mbe.fffEnergy_MeV)).flatten.toMap
    }
  ...
  def handleRunning(output: Output, procedure: Procedure) = {
    ...
    try {
      ...
      case e: Throwable => {
        logger.error(
          "Unexpected exeception while wrapping up unterminated procedure after service restart: " +
            e.getMessage + "\n    Output: " + output + "\n    Procedure: " + procedure
        )
      }
    }
