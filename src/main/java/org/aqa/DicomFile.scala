...
case class DicomFile(file: File) extends Logging {
  ...
  lazy val valid: Boolean = readResult.isRight
  ...
  def al: AttributeList = attributeList.get

  private lazy val standardImage: Option[BufferedImage] = {
  ...
  private lazy val maxContrastImage: Option[BufferedImage] = {
    if (valid && DicomUtil.isImageStorage(attributeList.get)) {
  ...
  private lazy val originalDicomImage: Option[DicomImage] = {
    attributeList match {
      case Some(_) => Some(new DicomImage(attributeList.get))
      case _       => None
    }
  }
