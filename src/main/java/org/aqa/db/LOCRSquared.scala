...
object LOCRSquared extends Logging {
  class LOCRSquaredTable(tag: Tag) extends Table[LOCRSquared](tag, "rSquared") {
    ...
    //noinspection SpellCheckingInspection
    private def rSquared_mmsq = column[Double]("rSquared_mmsq")
  ...
  /**
    * Get a list of all r-Squared-s for the given output
    */
  ...
  def insertSeq(list: Seq[LOCRSquared]): Unit = {
    ...
    val pkList = list.map(_.insertOrUpdate())
    logger.info("Number of rows inserted: " + pkList.size)
  }
