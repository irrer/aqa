...
case class BaselineContent(
  ...
  def insert: BaselineContent = {
    val insertQuery =
      BaselineContent.query returning BaselineContent.query.map(_.baselineContentPK) into ((baselineContent, baselineContentPK) => baselineContent.copy(baselineContentPK = Some(baselineContentPK)))
