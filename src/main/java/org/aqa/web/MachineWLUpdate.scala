...
/**
  * Provide administrative user interface for setting the Winston Lutz parameters on a per-machine basis.
  */

object MachineWLUpdate {
  def machineReference(machinePK: Long): String = { "/" + SubUrl.admin + "/" + "MachineWLUpdate" + "?" + MachineUpdate.machinePKTag + "=" + machinePK }
...
class MachineWLUpdate extends Restlet with SubUrlAdmin {

  private val pageTitleEdit = "Edit Winston Lutz Parameters"

  private val pageHeader = new WebPlainText("Winston Lutz Parameters", false, 6, 0, formatHeader)

  private val passLimit = new WebInputText("Pass Limit (mm)", 2, 0, "")

  private val ballDiameter = new WebInputText("Ball Diameter (mm)", 2, 0, "")
  ...
  private val machineWLPK = new WebInputHidden(MachineUpdate.machinePKTag)

  private val formEdit = new WebForm(pathOf, List(List(pageHeader), List(passLimit), List(ballDiameter), List(saveButton, cancelButton, deleteButton, machineWLPK)))

  private def formatHeader(valueMap: ValueMapT): Elem = {
    try {
      val machine = Machine.get(valueMap(machineWLPK.label).toLong).get
  ...
  private def validateWarningMax(valueMap: ValueMapT): StyleMapT = {
    0 match {
      case _ if valueMap(ballDiameter.label).trim.isEmpty => Error.make(ballDiameter, "Ball diameter can not be empty")
      case _ if !isPosDbl(valueMap(ballDiameter.label))   => Error.make(ballDiameter, "Ball diameter must be a floating point value greater than zero.")
      case _                                              => styleNone
    }
  }
  ...
  private def validateMachine(valueMap: ValueMapT): StyleMapT = {
    try {
      val machPK = valueMap(machineWLPK.label).toLong
  ...
  private def validateAuthorization(valueMap: ValueMapT): StyleMapT = {
    if (userIsWhitelisted(valueMap)) styleNone
      ...
      val machPK = valueMap(machineWLPK.label).trim.toLong
  ...
  /**
    * Create a new MachineWL from the value map
    */
  private def createMachineWLFromParameters(valueMap: ValueMapT): MachineWL = {
    // make the machWLPK value the same as the machinePK value
    val machWLPK = valueMap(machineWLPK.label).trim.toLong

    val mdq = new MachineWL(
      Some(machWLPK), // this is the same as the <code>Machine.machinePK</code>.
      passLimit.getValOrEmpty(valueMap).trim.toDouble,
      ballDiameter.getValOrEmpty(valueMap).trim.toDouble
    )
    mdq
  }
  ...
  private def saveEdits(valueMap: ValueMapT, response: Response): Unit = {
    ...
    if (styleMap.isEmpty) {
      createMachineWLFromParameters(valueMap).insertOrUpdate()

      // MachineUpdate.redirect(machPK, response)
      MachineWLList.redirect(response)
  ...
  /**
    ...
    */
  private def populateValueMap(machWL: MachineWL): ValueMapT = {
    Map(
      (machineWLPK.label, machWL.machineWLPK.get.toString),
      (passLimit.label, machWL.passLimit_mm.toString),
      (ballDiameter.label, machWL.ballDiameter_mm.toString)
    )
  }
  ...
  private def emptyForm(valueMap: ValueMapT, response: Response): Unit = {
    val valMap = getReference(valueMap) match {
      case Some(machWL) => populateValueMap(machWL)
  ...
  private def edit(inst: MachineWL, response: Response): Unit = {
    val valueMap = Map(
      (machineWLPK.label, inst.machineWLPK.get.toString),
      (passLimit.label, inst.passLimit_mm.toString),
      (ballDiameter.label, inst.ballDiameter_mm.toString)
    )
    formEdit.setFormResponse(valueMap, styleNone, pageTitleEdit, response, Status.SUCCESS_OK)
  }

  private def getReference(valueMap: ValueMapT): Option[MachineWL] = {
    val value = valueMap.get(machineWLPK.label)
    try {
      if (value.isDefined) {
        val inst = MachineWL.get(value.get.toLong)
        if (inst.isDefined) {
          inst
        } else
          None
  ...
  private def delete(valueMap: ValueMapT, response: Response): Unit = {
    ...
    if (auth.isEmpty) {
      val dailyQAPK = valueMap.get(machineWLPK.label)
      if (dailyQAPK.isDefined) {
        MachineWL.delete(dailyQAPK.get.toLong)
        MachineWLList.redirect(response)
      }
    } else {
  ...
  override def handle(request: Request, response: Response): Unit = {
    ...
    try {
      0 match {
        case _ if buttonIs(valueMap, cancelButton) => MachineWLList.redirect(response) //  MachineUpdate.redirect(valueMap(machineWLPK.label).toLong, response)
