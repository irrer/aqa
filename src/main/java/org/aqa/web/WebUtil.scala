...
object WebUtil extends Logging {
  ...
  def wrapAlias(text: String): Elem = {
    // @formatter:off
    <span aqaalias="">{text}</span>
    // @formatter:on
  }

  def wrapAlias(elem: Elem): Elem = {
    // @formatter:off
    <span aqaalias="">{elem}</span>
    // @formatter:on
  }
    ...
    <div title="Click '+' or '-' to show more or fewer digits of precision.">
      <table style="border:1px solid grey;">
        <tr>
          ...
          <td>
            <button style="margin:8px;" onclick="precisionInc()">
              <b>+</b>
            </button>
          </td>
          <td>
            <button style="margin:8px;" onclick="precisionDec()">
              <b>-</b>
            </button>
          </td>
        </tr>
   ...
   * @return Clone of original element with precision attribute set.
   */
  ...
  class WebInputCheckbox(override val label: String,
    ...
    override def toHtml(valueMap: ValueMapT, errorMap: StyleMapT, response: Option[Response], htmlAttrMap: Map[String, String] = htmlAttrMapP): Elem = {
      val input = {
        ...
        val i5 = {
          if (submitOnChange)
            addAttributeMap(i4, Map("onchange" -> "this.form.submit()"))
          else
            i4
        }
