...
case class WLBallOld(
  ...
  private def singleMax(spline: CubicSpline, values: IndexedSeq[Float]): Boolean = {
    ...
    if (crsCount == 2) {
      ...
      logger.error(" Wrong number of times that the ball spline crossed the average value.  Should be 2 but was " + crsCount)
      false
    }
  }
  ...
  def findBallCenter(): Option[(Double, Double)] = {
    ...
    fineBallLocate(ballRoi) match {
      case Some(loc: (Double, Double)) =>
        if (true) {
          val rect = new Rectangle(coarseCenter._1 - BALL_RADIUS, coarseCenter._2 - BALL_RADIUS, BALL_RADIUS * 2, BALL_RADIUS * 2)
          val ballImage = new DicomImage(ballAoi).getSubimage(rect)
          val wlBallValidation = WLBallValidation(new DicomImage(ballAoi), ballImage, rect, subDir)
          Trace.trace(wlBallValidation.validate)
        }
