...
import org.aqa.webrun.wl.wlMonthly.WLXlsxUtil.addFormulaCell
...
case class WLPreprocess(extendedData: ExtendedData, runReq: WLRunReq, dbList: Seq[WinstonLutz], workbook: SXSSFWorkbook) extends WLSheetMaker with Logging {
  ...
  private val mainTableColumnList: Seq[Col] = Seq(
    new ColString("A"),
    new ColString("B"),
    new ColString("C"),
    new ColAngle("E"),
  ...
  private def mainTableHeaders(): Elem = {
    ...
    <tr>
      ...
      {mainTableColumnList.map(addColHeader)}
    </tr>
  }

  private def makeMainTableRow(i: Int): Elem = {
    val row = sheet.createRow(sheet.getLastRowNum + 1)
    val list = mainTableColumnList.map(col => col.addCell(row))
    <tr>
  ...
  private def mainTable(): Seq[Elem] = {

    val rowList = (firstRow + 1 to lastRow).map(i => makeMainTableRow(i))
