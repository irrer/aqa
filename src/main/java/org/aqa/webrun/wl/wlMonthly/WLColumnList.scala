...
import org.aqa.Util
import org.aqa.webrun.wl.wlMonthly.WLXlsxUtil.acq
...
case class WLColumnList(machine: Machine, firstDateTime: Date) {
  ...
  val columnList: Seq[WLColumn] = {
    val list: Seq[WLColumn] = Seq(
      // @formatter:off
      new WLColumnText("machine id",                           (_: WinstonLutz, _: AttributeList) => machine.getRealId),
      ...
      new WLColumnAlNumeric("table yaw",                       TagByName.PatientSupportAngle),
      new WlColumnWlNumeric("gantry angle",                    _.gantryAngle_deg),
      new WlColumnWlNumeric("coll angle",                      _.collimatorAngle_deg),
