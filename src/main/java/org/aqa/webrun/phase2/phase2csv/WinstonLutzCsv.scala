...
class WinstonLutzCsv(metadataCache: MetadataCache) extends Phase2Csv[WinstonLutz.WinstonLutzHistory](metadataCache: MetadataCache) {
  ...
  override protected def makeColList: Seq[CsvCol[WLH]] = {
    Seq(
      ...
      CsvCol("Ball X mm", "Measured X position of ball in mm.", (wlh: WLH) => wlh.winstonLutz.ballCenterX_mm),
      CsvCol("Ball Y mm", "Measured Y position of ball in mm.", (wlh: WLH) => wlh.winstonLutz.ballCenterY_mm),
