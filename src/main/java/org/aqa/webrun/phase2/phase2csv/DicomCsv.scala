...
class DicomCsv {
  ...
  val colList: Seq[CsvCol[AL]] = Seq(
    ...
    CsvCol(
      "b value for converting pixel values to CU: CU = m*PixelValue + b",
      "Rescale Intercept.  DICOM 0028,1052",
      (al: AL) => Util.attr2Csv(al, TagByName.RescaleIntercept).head
    ),
    CsvCol(
      "m value for converting pixel values to CU: CU = m*PixelValue + b",
      "Rescale Slope.  DICOM 0028,1053",
      (al: AL) => Util.attr2Csv(al, TagByName.RescaleSlope).head
    ),
