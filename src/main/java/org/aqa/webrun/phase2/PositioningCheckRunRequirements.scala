 ...
 */
case class PositioningCheckRunRequirements(machine: Machine, sessionDir: File, plan: DicomFile, imageIdFileList: Seq[PositioningCheckFile])
  ...
  override def reDir(dir: File) = {
    ...
    new PositioningCheckRunRequirements(machine, sessionDir, newPlan, imageIdFileList.map(ii => ii.reDir(dir)))
  }
}
