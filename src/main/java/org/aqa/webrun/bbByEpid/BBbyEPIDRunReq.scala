...
case class BBbyEPIDRunReq(rtplanDicomFile: Either[DicomFile, AttributeList], imageList: Seq[DicomFile], machine: Machine) {
  ...
  def reDir(dir: File): BBbyEPIDRunReq = {
    val imageReg2 = imageList.map(image => image.reDir(dir))
