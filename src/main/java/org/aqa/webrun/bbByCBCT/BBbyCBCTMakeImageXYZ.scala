...
object BBbyCBCTMakeImageXYZ {
  ...
  def makeImagesXYZ(entireVolume: DicomVolume, fineLocation_vox: Point3d, voxSize_mm: Point3d): Seq[BufferedImage] = {
    ...
    val maxSize = new Point3i(((mm * 2) / voxSize_mm.getX).ceil.toInt, ((mm * 2) / voxSize_mm.getY).ceil.toInt, ((mm * 2) / voxSize_mm.getZ).ceil.toInt)

    // number of planes in each sub-volume  Make sure this is within the boundaries of the volume.
    val size = new Point3i(Math.min(entireVolume.xSize - start.getX, maxSize.getX), Math.min(entireVolume.ySize - start.getY, maxSize.getY), Math.min(entireVolume.zSize - start.getZ, maxSize.getZ))
